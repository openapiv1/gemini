...
const formatModelName = (model: string) => {
  return model
    ...
    .map((part) => {
      ...
      if (part === "pro" || part === "flash" || part === "lite") {
...
const Header: React.FC<HeaderProps> = ({ selectedModel, onModelChange, onToggleSidebar }) => {
      ...
      <div className="w-full">
        <div className="flex items-center justify-start h-8">
            <button
                ...
                className="p-1 mr-2 text-black dark:text-white"
          ...
          <div className="relative" ref={dropdownRef}>
            <div className="flex items-center justify-start h-8">
              <button
                ...
                className="flex items-center gap-1 text-xl text-black dark:text-white"
              >
            ...
            {isOpen && (
              <div className="absolute top-full mt-2 w-auto bg-white dark:bg-[#212121] rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-700 z-10">
                <ul className="py-1">
                  {models.map((model) => (
                    <li
                      key={model}
                      onClick={() => handleModelSelect(model)}
                      className="px-4 py-2 text-base text-black dark:text-white hover:bg-zinc-100 dark:hover:bg-[#424242] cursor-pointer"
                    >
        ...
        <div className="mt-2 h-px w-full bg-gradient-to-r from-transparent via-transparent to-transparent"></div>
      </div>
    </header>
  );
};
