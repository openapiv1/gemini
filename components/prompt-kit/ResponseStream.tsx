...
export function ResponseStream({
  ...
  useEffect(() => {
    ...
    if (displayedText.length < textStream.length) {
      const timeoutId = setTimeout(() => {
        setDisplayedText(textStream.slice(0, displayedText.length + 1));
      }, speed);
      return () => clearTimeout(timeoutId);
    }
  }, [textStream, displayedText, speed, isStopped]);

  useEffect(() => {
    const isAnimationFinished = displayedText.length === textStream.length && textStream.length > 0;
    if ((isAnimationFinished || isStopped) && !completeCalledRef.current) {
      onComplete?.();
  ...
  }, [displayedText, textStream, isStopped, onComplete]);
  ...
  return (
    <div className={className}>
      <div><ReactMarkdown>{displayedText}</ReactMarkdown></div>
    </div>
  );
}